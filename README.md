...
3. 사용 방법
	1. local_ai.py 파일 실행(Ex. powershell 에서 실행 : python3.13.exe .\local_ai.py)
	2. 슬랙에서 앱 추가
...
4. 봇 권한(Scopes) 검문 및 봇 토큰 발급:
   ...
   - 하단의 [Scopes] ➔ [Bot Token Scopes] 구역에 app_mentions:read, chat:write, files:write, groups:history, im:history, channels:history 권한이 인라인 정책으로 들어가 있는지 확인 후 없다면 수동 추가합니다.
